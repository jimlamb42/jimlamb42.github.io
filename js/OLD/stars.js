 ...
 * Created by James on 4/12/2015.
    ...
    3. cube length and width slight random modifier
    4. sprite size slight random modifier

    1. Loading not going away
    2. make particle movement independent of scale to avoid global movement
    4. Morph center 3 balls
    ...
    Questions:
    1. better way to move a value positive and negative ex. camera movement
    2. Smoother way to move the variable, used map previously but does not work due
...
//creates camera
var camera = new THREE.PerspectiveCamera( 80, window.innerWidth / window.innerHeight, 0.1, 250 );
//creates rednder
...
scene.add( light );
var pointLight = new THREE.PointLight( 0xCCCCDD, 100, 200 );
pointLight.position.set( 0, 0,zView -10 );
...
//(3000,...
var tunnelGeometry	= new THREE.CylinderGeometry( 500, 0, 350, 32, 20, true );
...
//(x,1000,x)
scene.fog	= new THREE.Fog( 0x000000, 200,250 );
...
function edgeGlow(vector) {
    ...
    glowSprite.scale.set(90, 90, 0.1);
    glowSprite.position.copy(vector);
    scene.add(glowSprite); // this centers the glow at the mesh
}
var ballV1 =new THREE.Vector3(0,0,100);
var ballV2 = new THREE.Vector3(40,0,100);
var ballV3 = new THREE.Vector3(-40,0,100);
...
function makeSphere(geometry) {
    // bring down to single material if fps is low
...
sphere = makeSphere(marchingCubes(points,values,0.5));
scene.add(sphere);
...
function render() {
   ...
   // particleGroup.rotation.z = time * .75;
    ...
    if(load){
        loadingSprite.position.set( 0, -window.innerHeight/2, 110 );
        loadingSprite.scale.set( 80,30, 1.0 ); // imageWidth, imageHeight
...
//Moves the camera from side to side
  camera.lookAt(sphere.position);
    ...
    if (camera.position.x < 40 && right) {
        camera.position.x += .2;
    }
    if (camera.position.x > -40 && right == false){
        camera.position.x -= .2;
    }
    if (upper){
        camera.position.y +=.2;
